...
function auth(roles) {
  return (req, res, next) => {
    ...
    const user = claims && db.USERS[claims.email];
...
const credits = (code) => db.COURSES[code]?.credits || 0;
...
app.get('/api/me', auth(['student']), (req, res) => {
  ...
  res.json({
    ...
    completed: u.completed.map((c) => ({ ...db.COURSES[c.code], grade: c.grade, pts: c.pts })),
    remaining: u.remaining.map((code) => ({ ...db.COURSES[code], open: u.open.includes(code) })),
...
app.get('/api/courses/open', auth(['student']), (req, res) => {
  ...
  const list = u.open
    ...
    .filter((code) => db.COURSES[code].prereqs.every((p) => passed.has(p)))
    .map((code) => ({ ...db.COURSES[code], cost: cost(code) }));
...
app.post('/api/register', auth(['student']), (req, res) => {
  ...
  db.registrations[u.email] = codes;
...
app.get('/api/schedule', auth(['student']), (req, res) => {
  const codes = db.registrations[req.user.email] || [];
  const subjects = codes.map((code) => ({
    ...db.COURSES[code],
    cost: cost(code),
    sessions: db.OFFERINGS[code] || [],
  }));
  res.json({
...
app.get('/api/exams', auth(['student']), (req, res) => {
  if (!db.term.examPeriod) return res.json({ examPeriod: false, term: db.term.name, exams: [] });
  const codes = db.registrations[req.user.email] || [];
  const exams = codes.map((code) => ({ ...db.COURSES[code], ...db.EXAMS[code] }));
  res.json({ examPeriod: true, term: db.term.name, studentId: req.user.id, exams });
});

/* ======================= STAFF ENDPOINTS ====================== */
app.get('/api/staff/courses', auth(['staff']), (req, res) => {
  const courses = req.user.teaching.map((t) => ({ ...t, ...db.COURSES[t.code] }));
  res.json({ dept: req.user.dept, courses });
});

app.get('/api/staff/roster', auth(['staff']), (req, res) => {
  const rosters = req.user.teaching.map((t) => ({
    ...db.COURSES[t.code], slot: t.slot, room: t.room, enrolled: t.students,
    students: db.SAMPLE_ROSTER,
  }));
  res.json({ rosters });
});

app.get('/api/staff/exams', auth(['staff']), (req, res) => {
  if (!db.term.examPeriod) return res.json({ examPeriod: false, exams: [] });
  const exams = req.user.teaching.map((t) => ({ ...db.COURSES[t.code], students: t.students, ...db.EXAMS[t.code] }));
  res.json({ examPeriod: true, exams });
});

/* ======================= TERM (demo) ========================= */
app.get('/api/term', auth(), (req, res) => res.json(db.term));
app.post('/api/term/exam-period', auth(), (req, res) => {
  db.term.examPeriod = !!req.body?.on;
  res.json(db.term);
});
